#Script to Process EC data with Ready Proc
#   1. load data and initialize refernce class
#   2. Ustar filtering
#   3. gap fill
#   4. partition fluxes
#   5. save
...
#+++ Load data with one header and one unit row from (tab-delimited) text file
EddyData.F <- fLoadTXTIntoDataframe('out/ogletreeRprocIn.txt')
...
#+++ Initalize R5 reference class sEddyProc for processing of eddy data
#+++ with all variables needed for processing later
EddyProc.C <- sEddyProc$new('Ogletree', EddyDataWithPosix.F, c('NEE','Rg','Tair','VPD', 'Ustar'))
...
EddyProc.C$sMRFluxPartition(Lat_deg.n=51.0, Long_deg.n=13.6, TimeZone_h.n=1, Suffix.s='WithUstar')  # Note suffix
  ...
  EddyProc.C$sPlotDailySumsY('NEE_f','NEE_fsd', Year.i=2016) #Plot of sums with uncertainties
